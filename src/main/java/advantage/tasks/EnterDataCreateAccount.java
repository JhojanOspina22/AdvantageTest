...
public class EnterDataCreateAccount implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(Enter.theValue(email).into(CreateAccountPage.INP_EMAIL));
        actor.attemptsTo(Enter.theValue(password).into(CreateAccountPage.INP_PASSWORD));
        actor.attemptsTo(Enter.theValue(confirmPassword).into(CreateAccountPage.INP_CONFIRM_PASSWORD));
        WaitUntil.the(CreateAccountPage.CHK_I_AGREE, WebElementStateMatchers.isClickable()).forNoMoreThan(30).seconds();
        actor.attemptsTo(Click.on(CreateAccountPage.CHK_I_AGREE));
        WaitUntil.the(CreateAccountPage.BTN_REGISTER, WebElementStateMatchers.isEnabled()).forNoMoreThan(30).seconds();
        actor.attemptsTo(Click.on(CreateAccountPage.BTN_REGISTER));
    }
